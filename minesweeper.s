...
#
#
# This program was written by Anson Lai (z5361987)
# on October 2021
#
...
rg_loop1:                               # for (int row = 0; row < N_ROWS; row++) {
        bge     $t0, N_ROWS, exit_rg_loop1
        li      $t1, 0
rg_loop2:                               # for (int col = 0; col < N_COLS; col++) {
        bge     $t1, N_COLS, exit_rg_loop2
                                        
        la      $t2, grid               # grid[row][col] |= IS_RVLD_MASK;
        mul     $t3, $t0, N_COLS
        add     $t4, $t3, $t1
        add     $t5, $t4, $t2
        lb      $t6, ($t5)
...
place_bombs__body:
        ...
        li      $s0, 0                  # i in $s0
        move    $s1, $a0                # bad_row in $a0 in $s1
        move    $s2, $a1                # bad_col in $a1 in $s2
          
        lw      $s3, total_bombs
pb_loop:                                # for (int i = 0; i < total_bombs; i++) {
        bge     $s0, $s3, exit_pb_loop
...
mark_cell__body:
        ...
        # row in $a0
        # col in $a1        
        la      $t0, grid               # calculate grid[row][col]
        mul     $t1, $a0, N_COLS
        add     $t2, $t1, $a1
        add     $t3, $t0, $t2
        lb      $t4, ($t3)           
...
if_revealed:
        beqz    $t5, end_if_revealed    # if (grid[row][col] & IS_RVLD_MASK) {
        
        lb      $t0, debug_mode
if_debug:                               # if (debug_mode) {        
        beqz    $t0, end_debug
...
end_if_revealed:        #   }
        ...
        and     $t5, $t4, IS_MRKD_MASK
if_marked:                              # if (grid[row][col] & IS_MRKD_MASK) {
        beqz    $t5, else_marked        
        
        li      $t0, IS_MRKD_MASK       # grid[row][col] = grid[row][col] & ~IS_MRKD_MASK
        not     $t1, $t0
        and     $t5, $t4, $t1
        sb      $t5, ($t3)
        
        la      $t0, bomb_count         # bomb_count++;
        lw      $t1, bomb_count
        addi    $t1, $t1, 1       
        sw      $t1, ($t0)
...
else_marked:                            # } else {
        ...
        or      $t4, $t4, IS_MRKD_MASK  # grid[row][col] |= IS_MRKD_MASK;
        sb      $t4, ($t3)
...
reveal_cell__body:
        ...
        # row in $a0
        # col in $a1               
        la      $t0, grid               # grid[row][col] in $t4
        mul     $t1, $a0, N_COLS        # $t3 is the address
        add     $t2, $t1, $a1
        add     $t3, $t0, $t2
        lb      $t4, ($t3)     
                
        move    $s0, $t4                # store $t4 into $s0
        move    $s1, $t3                # store $t3 into $s1
        move    $s2, $a0                # store $a0 (row) into $s2
        move    $s3, $a1                # store $a1 (col) into $s3

                                        # // Trigger game over if the cell is a bomb.
        and     $t0, $s0, IS_BOMB_MASK
if_gameover:
        beqz    $t0, end_gameover       # if (grid[row][col] & IS_BOMB_MASK) {
        la      $t1, game_state         # game_state = LOSE;
        li      $t2, LOSE
        sw      $t2, ($t1)
end_gameover: 

                                        # // Cannot reveal a cell that is currently marked.
        and     $t0, $s0, IS_MRKD_MASK
if_rc_marked:     
        beqz    $t0, end_rc_marked      # if (grid[row][col] & IS_MRKD_MASK) {        
        lb      $t1, debug_mode
if_rc_debug:        
        beqz    $t1, end_rc_debug       # if (debug_mode) {
...
end_rc_marked:  
        ...
        and     $t0, $s0, IS_RVLD_MASK       
if_rc_revealed:      
        beqz    $t0, end_rc_revealed    # if (grid[row][col] & IS_RVLD_MASK) {
        
        lb      $t1, debug_mode
if_rc_debug1:        
        beqz    $t1, end_rc_debug1      # if (debug_mode) {
        j       reveal_cell__epilogue   # return;            
end_rc_debug1:    
...
end_rc_revealed:     
                                        ...
                                        # // Reveal the cell.
        and     $t0, $s0, VALUE_MASK
if_rc_rvlcell:
        bnez    $t0, else_rc_rvlcell    # if ((grid[row][col] & VALUE_MASK) == 0) {
       
        move    $a0, $s2                # clear_surroundings(row, col);
        move    $a1, $s3
        jal     clear_surroundings  
        
        j       end_rc_rvlcell 
else_rc_rvlcell:                        # } else {
        or      $s0, $s0, IS_RVLD_MASK  # grid[row][col] |= IS_RVLD_MASK;
        sb      $s0, ($s1)
        lw      $t1, game_state
        li      $t2, LOSE
if_rc_gamestate:        
        beq     $t1, $t2, end_rc_gamestate # if (game_state != LOSE) {
        la      $t3, cells_left         # cells_left--;
        lw      $t4, cells_left
        addi    $t4, $t4, -1
        sw      $t4, ($t3)
end_rc_gamestate:      
end_rc_rvlcell:        
        ...
        lw      $t4, cells_left 
if_rc_nocell:
        bnez    $t4, end_rc_nocell      # if (cells_left == 0) {
        
        la      $t5, game_state         # game_state = WIN;
        li      $t6, WIN
        sw      $t6, ($t5)
end_rc_nocell:        #   }
...
clear_surroundings__body:
        ...
        bltz    $s2, clear_surroundings__epilogue      # goto "if_cs" if row < 0
        bge     $s2, $t0, clear_surroundings__epilogue # row >= N_ROWS
        bltz    $s3, clear_surroundings__epilogue      # col < 0
        bge     $s3, $t1, clear_surroundings__epilogue # col >= N_COLS
     ...
     # }
        ...
        and     $t0, $s0, IS_RVLD_MASK
if_cs_revealed:                                # if (grid[row][col] & IS_RVLD_MASK) {
        beqz    $t0, end_cs_revealed
...
end_cs_revealed:        #   }
                                               ...
                                               # // Reveal the cell.
        or      $t0, $s0, IS_RVLD_MASK         # grid[row][col] |= IS_RVLD_MASK;
        sb      $t0, ($s1)
        move    $s0, $t0
        ...
        li      $t0, IS_MRKD_MASK              # grid[row][col] &= ~IS_MRKD_MASK;
        not     $t1, $t0
        and     $t2, $s0, $t1
        sb      $t2, ($s1)
        move    $s0, $t2
        ...
        and     $t0, $s0, VALUE_MASK
if_cs_stopr:                                   # if (grid[row][col] & VALUE_MASK) {
        beqz    $t0, end_cs_stopr
        j       clear_surroundings__epilogue   # return;
end_cs_stopr:                                                          
...
update_highscore__body:
        ...
        # score in $a0
        lw      $t0, high_score
if_hs:                                          # if (high_score.score < score) {
        bge     $t0, $a0, end_hs
        
        la      $t1, high_score                 # high_score.score = score;
        sw      $a0, ($t1)
       
                                                # // Copy over the string.
        li      $t2, 0                          # int i = 0;        
loop_hs:                                        # while (user_name[i] != '\0') {
        la      $t3, user_name  
        add     $t5, $t2, $t3        
        lb      $t4, ($t5)   

        beqz    $t4, exit_loop_hs
        
        addi    $t6, $t1, 4                     # high_score.name

        add     $t6, $t6, $t2       
        sb      $t4, ($t6)                      # high_score.name[i] = user_name[i];
        
        addi    $t2, $t2, 1                     # ++i;
        j       loop_hs
exit_loop_hs:        
        
        addi    $t6, $t1, 4                     # high_score.name[i] = '\0';
        mul     $t7, $t2, 4
        add     $t6, $t6, $t7
        sb      $zero, ($t6)
...
print_scores__body:
        ...
        li      $t0, 0                  # int i = 0
        li      $t1, MAX_SCORES   
loop_ps:                                # for (int i = 0; i < MAX_SCORES; i++) {
        bge     $t0, $t1, exit_loop_ps
        
        la      $t2, scores             # struct UserScore curr = scores[i];
        mul     $t3, $t0, USER_SCORE_SIZE
        add     $t4, $t3, $t2
        lw      $t5, ($t4)              # $t5 the location of score[i]
        
       
                
        li      $t7, -1
if_ps:                                  # if (curr.score == -1) {
        bne     $t5, $t7, end_ps
        j       exit_loop_ps            # break;
end_ps:      
